...
const SLASH_COMMANDS: SlashCommand[] = [
  ...
  },
  {
    ...
    description: "Paste a job description — I'll tailor your CV, find the HR contact, and draft your application email for review",
    icon: Briefcase,
    action: "fill",
    template: "",
    pipeline: "apply_jd",
  ...
  {
    name: "auto",
    label: "Auto-apply pipeline",
    description: "Search for jobs by role/location, tailor CV, find HR contacts & auto-send applications",
    icon: Zap,
    action: "fill",
    template: "",
...
export function CenterPanel({ activeSessionId, onSessionCreated }: CenterPanelProps) {
  ...
  const [floatingEditMeta, setFloatingEditMeta] = useState<Record<string, any> | null>(null);
  ...
  const sendAction = async (action: string) => {
    ...
    try {
      ...
      if (resp.metadata?.type === "cv_review" && resp.metadata?.auto_open_edit) {
        setFloatingEditMeta(resp.metadata);
        return;
      }
  ...
  return (
    <main className="flex flex-col h-full bg-white relative">
      ...
      {/* Floating edit modal — renders outside chat history so no card appears in the feed */}
      {floatingEditMeta && (
        <div className="hidden">
          <CVReviewCard
            key={floatingEditMeta.tailored_cv_id}
            metadata={floatingEditMeta as any}
            onSendAction={(action) => {
              setFloatingEditMeta(null);
              sendAction(action);
            }}
          />
        </div>
